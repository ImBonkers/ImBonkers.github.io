...
function draw_image(x, y, offset, canvas, img){
    var ratio = canvas.width / canvas.height
    section_width = canvas.width / SECTION_COUNT
...
function update_map(canvas, zoom, offset){
    ...
    var region = {
        ...
        upper_x:boundary.lower + Math.ceil(offset.x * ZOOM_SECTIONS[zoom]) + SECTION_COUNT,
        ...
        for (let x = region.lower_x; x < region.upper_x; x++) {
            for (let y = region.lower_y; y < region.upper_y; y++) {
                if (!sections[zoom][[x,y]]){
                    load_image(get_url(x, y, zoom)).then(img => {
                        sections[zoom][[x, y]] = img;
                        draw_image(x-offset.x*Math.pow(2, zoom), y-offset.y*Math.pow(2, zoom), offset, canvas, img);
                        // draw_offset(offset, zoom, canvas);
                        // draw_section(x, y, zoom, canvas, offset);
                    });
                }
                else{
                    draw_image(x-offset.x*Math.pow(2, zoom), y-offset.y*Math.pow(2, zoom), offset, canvas, sections[zoom][[x, y]]);
                }
            }
        }
    });
